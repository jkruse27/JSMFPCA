...
class JSMFPCA(FunctionalEstimator):
    ...
    def fit(
        ...
        selection = self.selector.select(
            estimator=self.spectral,
            dataset=circadian_scores,
        )
        ...
        prior = self.prior_builder.build(
            self.spectral,
        )

        inference = SpectralInference(
            basis=self.spectral.basis_,
            model=self.spectral,
        ...
        self.fingerprint_builder.retained_components = {
            h + 1: k
            for h, k in enumerate(selection.n_components)
        }

        subjects = inference.estimate_dataset(
            circadian_scores,
            noise_covariance=self.spectral.noise_covariance_,
        )

        fingerprints = self.fingerprint_builder.transform_dataset(
            subjects
        )
    ...
    def transform(
        ...
        return self.fingerprint_builder.transform_dataset(
            subjects
        )
    ...
    def reconstruction_error(
        ...
        return predicted.reconstruction_error(
            observed
        )
