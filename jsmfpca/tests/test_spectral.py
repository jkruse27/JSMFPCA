"""
Tests for Stage 2 (Spectral model).
"""
...
def test_transform_runs(synthetic_dataset):
    ...
    _, _, model, dataset, transformed = _fit_spectral(
        synthetic_dataset
    )
...
def test_cross_spectrum_shape(synthetic_dataset):
    ...
    _, _, model, dataset, _ = _fit_spectral(
        synthetic_dataset
    )
...
def test_cross_spectra_are_hermitian(
    synthetic_dataset,
):
...
def test_shrunk_spectra_are_hermitian(
    synthetic_dataset,
):
...
def test_subject_coefficients_shape(
    synthetic_dataset,
):
...
def test_rotated_coefficients_shape(
    synthetic_dataset,
):
...
def test_rotation_preserves_energy(
    synthetic_dataset,
):

    _, _, model, _, transformed = _fit_spectral(
        synthetic_dataset
    )
...
def test_inverse_rotation(
    ...
    for r, U in enumerate(model.eigenvectors_):
        ...
        reconstructed[i] = (
            subject.rotated_coefficients[i]
            @ U.conj().T
        )

        reconstructed[i + 1] = (
            subject.rotated_coefficients[i + 1]
            @ U.conj().T
        )
...
def test_reconstruction_shape(
    ...
    reconstructed = model.reconstruct_subject(
        subject,
    )
...
def test_subject_order_invariant(
    ...
    _, _, model1, dataset, _ = _fit_spectral(
        synthetic_dataset
    )
...
def test_diagonal_spectral_model(
    synthetic_dataset,
):
    ...
    circadian_scores = circadian.transform(
        scores
    )
