...
def test_transform_dimensions(synthetic_dataset):
    _, model, scores, transformed = _fit_stage1(synthetic_dataset)
...
def test_subjects_are_centered(synthetic_dataset):
    _, _, _, transformed = _fit_stage1(synthetic_dataset)
...
def test_reconstruction_identity(synthetic_dataset):
    ...
    np.testing.assert_allclose(
        reconstructed.as_array(), scores.as_array(), atol=1e-8
    )
...
def test_mean_plus_centered_equals_scores(synthetic_dataset):
    _, _, scores, transformed = _fit_stage1(synthetic_dataset)

    for s0, s1 in zip(scores.subjects, transformed.subjects):
        reconstructed = s1.mean + s1.centered
        np.testing.assert_allclose(reconstructed, s0.scores, atol=1e-10)
...
def test_hours_preserved(synthetic_dataset):
    ...
    for s0, s1 in zip(scores.subjects, transformed.subjects):
        np.testing.assert_array_equal(s0.hours, s1.hours)
...
def test_subject_ids_preserved(synthetic_dataset):
    _, _, scores, transformed = _fit_stage1(synthetic_dataset)

    ids0 = [s.subject_id for s in scores.subjects]
    ids1 = [s.subject_id for s in transformed.subjects]
...
def test_transform_is_deterministic(synthetic_dataset):
    _, model, scores, _ = _fit_stage1(synthetic_dataset)

    t1 = model.transform(scores)
    t2 = model.transform(scores)

    np.testing.assert_allclose(t1.as_array(), t2.as_array(), atol=1e-12)
...
def test_flat_subject_has_zero_centered():
    ...
    for subject in transformed.subjects:
        np.testing.assert_allclose(subject.centered, 0.0, atol=1e-12)
        np.testing.assert_allclose(subject.mean, subject.scores[0], atol=1e-12)
